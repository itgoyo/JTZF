...
class DownloadBot:
    ...
    def __init__(self):
        ...
        # 加载 skip_keywords.txt
        self.load_skip_keywords()
        
        # 加载自动命令
        self.load_auto_commands()

    def load_skip_keywords(self):
        """Load skip keywords from skip_keywords.txt"""
        skip_keywords_path = os.path.join(os.path.abspath("."), "skip_keywords.txt")
        if os.path.exists(skip_keywords_path):
            try:
                with open(skip_keywords_path, "r", encoding="utf-8") as f:
                    keywords = [line.strip() for line in f.readlines() if line.strip()]
                    self.skip_keywords = keywords
                    logger.info(f"Loaded {len(keywords)} skip keywords")
            except Exception as e:
                logger.warning(f"Failed to load skip_keywords.txt: {e}")
        else:
            logger.info("skip_keywords.txt not found, skip keywords filtering disabled")

    def load_auto_commands(self):
        """Load auto commands from command.txt"""
        command_path = os.path.join(os.path.abspath("."), "command.txt")
        if os.path.exists(command_path):
            try:
                with open(command_path, "r", encoding="utf-8") as f:
                    commands = [line.strip() for line in f.readlines() if line.strip() and not line.startswith("#")]
                    self.auto_commands = commands
                    logger.info(f"Loaded {len(commands)} auto commands")
            except Exception as e:
                logger.warning(f"Failed to load command.txt: {e}")
                self.auto_commands = []
        else:
            logger.info("command.txt not found, auto commands disabled")
            self.auto_commands = []

    async def execute_auto_commands(self):
        """Execute auto commands after bot startup"""
        if not hasattr(self, 'auto_commands') or not self.auto_commands:
            return
        
        logger.info("Executing auto commands...")
        for command in self.auto_commands:
            try:
                # 获取管理员用户ID
                ...
                mock_message = type('MockMessage', (), {
                    'text': command,
                    'from_user': type('MockUser', (), {'id': admin.id})(),
                    'chat': type('MockChat', (), {'id': admin.id})(),
                    'id': 0
                })()
                
                # 根据命令类型执行相应的处理函数
                if command.startswith("/listen_forward"):
                    await set_listen_forward_msg(self.bot, mock_message)
                    logger.info(f"Executed auto command: {command}")
                elif command.startswith("/download"):
                    await download_from_bot(self.bot, mock_message)
                    logger.info(f"Executed auto command: {command}")
                elif command.startswith("/forward"):
                    await forward_messages(self.bot, mock_message)
                    logger.info(f"Executed auto command: {command}")
                else:
                    logger.warning(f"Unknown auto command: {command}")
                
                # 添加延迟避免过快执行
                await asyncio.sleep(1)
                
            except Exception as e:
                logger.error(f"Failed to execute auto command '{command}': {e}")

    def check_skip_keywords(self, text: str) -> bool:
        """Check if text contains any skip keywords"""
        ...
        for keyword in self.skip_keywords:
            if keyword.lower() in text_lower:
                logger.info(f"Message skipped due to keyword: {keyword}")
                return True
        return False
    ...
    async def start(
        ...
        # 自动执行命令
        await self.execute_auto_commands()
...
async def forward_normal_content(
    ...
    if message_text and _bot.check_skip_keywords(message_text):
...
async def listen_forward_msg(client: pyrogram.Client, message: pyrogram.types.Message):
    ...
    if message.chat and message.chat.id in _bot.listen_forward_chat:
        ...
        if not node.has_protected_content:
            ...
            if message.media_group_id:
                # 检查是否已经处理过这个媒体组
                if not hasattr(node, 'processed_media_groups'):
                    node.processed_media_groups = set()
                
                # 如果这个媒体组已经处理过，跳过
                if message.media_group_id in node.processed_media_groups:
                    return
                
                # 标记这个媒体组正在处理
                node.processed_media_groups.add(message.media_group_id)
                ...
                for attempt in range(max_attempts):
                    try:
                        ...
                        media_group = await _bot.client.get_media_group(message.chat.id, message.id)
                        current_count = len(media_group) if media_group else 0
                        
                        if current_count > 1:
                            # 如果连续两次获取到的消息数量相同，说明媒体组已经稳定
                            if current_count == last_count:
                                stable_count += 1
                                if stable_count >= 2:  # 连续两次相同，认为媒体组完整
                                    logger.info(f"Media group stabilized with {current_count} messages after {attempt + 1} attempts")
                                    # 直接转发整个媒体组
                                    from module.pyrogram_extension import forward_messages
                                    await forward_messages(
                                        _bot.client,
                                        node.upload_telegram_chat_id,
                                        message.chat.id,
                                        [msg.id for msg in media_group],
                                        drop_author=True
                                    )
                                    await report_bot_status(client, node, immediate_reply=True)
                                    return
                            ...
                            logger.info(f"Attempt {attempt + 1}: Media group has {current_count} messages")
                        else:
                            logger.warning(f"Attempt {attempt + 1}: Media group has only {current_count} messages")
                    except Exception as e:
                        logger.warning(f"Attempt {attempt + 1}: Failed to get media group: {e}")
                
                # 如果多次尝试都失败，回退到单个消息处理
                node.processed_media_groups.discard(message.media_group_id)
                logger.warning("Failed to get complete media group, falling back to single message processing")
